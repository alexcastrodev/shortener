module RetryableJob
  ...
  def handle_retry(exception)
    options = arguments.last.is_a?(Hash) ? arguments.last.except(:retry_count) : {}
    retry_count = ((arguments.last.is_a?(Hash) && arguments.last[:retry_count]) || 0) + 1
    max_retries = 5
    ...
    if retry_count <= max_retries
      ...
      self.class.set(wait: wait_seconds.seconds).perform_later(*arguments.first(1), **options, retry_count: retry_count)
    else
